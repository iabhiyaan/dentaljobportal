...
class FrontController extends Controller
{
   ...
   public function apply(Request $request)
   {
      ...
      $mailSentToEmployer = Mail::send('email.job_application', $mail_data, function ($message) use ($mail_data, $job) {
         $message->to($job->employer->employer_email)->from($mail_data['email']);
         $message->subject('Job Application Received! ');
      });


      if ($mailSentToEmployer) {
         Mail::send('email.reply-from-emp', $mail_data, function ($message) use ($jobseeker, $job) {
            $message->to($jobseeker->email)->from($job->employer->employer_email);
            $message->subject('Job Application Received! ');
         });
      }
